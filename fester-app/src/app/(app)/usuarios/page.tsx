...
export default function UsuariosPage() {
  ...
  async function resetPassword(user_id: string, nombre: string) {
    const pwd = window.prompt('Nueva contrasena para ' + nombre + ':');
    if (!pwd) return;
    const res = await fetch('/api/usuarios', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
  ...
  const rolBadge = (r: string) => {
    if (r === 'admin') return 'bg-blue-100 text-fester-blue';
    if (r === 'gerente') return 'bg-indigo-100 text-indigo-700';
    if (r === 'encargado') return 'bg-purple-100 text-purple-700';
          ...
          <div>
            {form.rol === 'aplicador' && (
              <>
                ...
                <select className="input" value={form.aplicador_id} required onChange={(e) => setForm({ ...form, aplicador_id: e.target.value })}>
            ...
            {form.rol === 'gerente' && (
              <>
                ...
                <select className="input" value={form.zona_id} required onChange={(e) => setForm({ ...form, zona_id: e.target.value })}>
                  <option value="">-- Seleccionar --</option>
            ...
            {(form.rol === 'encargado' || form.rol === 'admin') && (
              <div className="pt-6 text-xs text-slate-400">Sin campo adicional.</div>
            )}
          </div>
